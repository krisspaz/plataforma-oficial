...
class ReciboController extends Controller
{
    public function generarRecibo(Request $request)
    {
        ...
        if ($request->pago_id) {
            ...
            $reciboExistente = ReciboEmitido::where('pago_id', $request->pago_id)->first();

            if ($reciboExistente) {

                // Si ya hay un recibo emitido con certificado FEL
                if (!empty($reciboExistente->link)) {
                    return response()->streamDownload(function () use ($reciboExistente) {
                        echo file_get_contents($reciboExistente->link);
                    }, "RECIBO-{$reciboExistente->numero}.pdf");
                }
        ...
        // Si no hay link válido, mostrar mensaje de error
        if (empty($response['link'])) {
            return redirect()->back()->with('error', 'No se recibió un enlace de descarga del certificado.');
        }

        // ✅ Descargar automáticamente el archivo desde la URL recibida
        return response()->streamDownload(function () use ($response) {
            echo file_get_contents($response['link']);
        }, "RECIBO-{$response['numero']}.pdf");
    ...
    private function agregarEmisor($w)
    {
        ...
        $w->writeAttribute("CodigoEstablecimiento", env('SOAP_ESTABLECIMIENTO_REC'));
        ...
        $w->writeElement("dte:CodigoPostal", "0001");
        $w->writeElement("dte:Municipio", "GUATEMALA");
        $w->writeElement("dte:Departamento", "GUATEMALA");
    ...
    private function agregarReceptor($w, $nit, $cliente, $direccion, $TipoEspecial)
    {
        ...
        $w->writeElement("dte:Municipio", "");
        $w->writeElement("dte:Departamento", "");
        $w->writeElement("dte:Pais", "GT");
    ...
    private function agregarItems($w, $items, &$totalIVA, &$granTotal)
    {
        ...
        $w->writeAttribute("CodigoEscenario", "8");
        $w->writeAttribute("TipoFrase", "4");
    ...
    {
        $recibo = ReciboEmitido::findOrFail($id);

        // o el path que usesfa
        return response()->download($recibo->link);
    }
