...
<div class="box box-default">
    ...
    <div class="box-header with-border">
        <h3 class="box-title">📄 Reporte de Recibos Internos Emitidos</h3>
    </div>

    <div class="box-body">
        {{-- Buscador por NIT --}}
        <form method="POST" action="{{ route('reportes.recibosinternos.buscar') }}" class="form-inline mb-3">
            @csrf
            ...
            <a href="{{ route('reportes.recibosinternos.index') }}" class="btn btn-default ml-2"><i class="fa fa-refresh"></i> Limpiar</a>
        </form>
        ...
        @endphp
          ...
          <div class="table-responsive">
            <table id="recibosITable" class="table table-striped table-bordered" style="width:100%">
                <thead>
                    <tr class="active">
                        <th>NIT</th>
                        <th>Serie</th>
                ...
                <tbody>
                    @forelse ($agrupadas as $numero => $grupo)
                        ...
                        >
                            ...
                            <td>
                                <a href="{{ route('reportes.recibosinternos.show', $item->serie) }}" class="btn btn-xs btn-primary">
                                    <i class="fa fa-eye"></i> Detalle
                                </a>
                                <a href="{{ route('reportes.recibosinternos.descargarPDF', $item->serie) }}" class="btn btn-xs btn-success">
                   ...
                   @empty
                    <tr>
                        <td colspan="10">
                            <div class="text-center">No hay registros disponibles.</div>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="modal fade" id="anularModal{{ $numero }}" tabindex="-1" role="dialog" aria-labelledby="anularModalLabel{{ $numero }}" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('reportes.recibosinternos.anular', $numero) }}" method="POST">
                    @csrf
                    @method('PUT')
