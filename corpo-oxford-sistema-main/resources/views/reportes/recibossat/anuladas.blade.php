...
@endif
    ...
    <div class="box-header with-border">
        <h3 class="box-title">📄 Recibos SAT para Anulación</h3>
    </div>
            ...
            <table class="table table-striped table-bordered">
                ...
                <tbody>
                    @forelse ($agrupadas as $numero => $grupo)
                        @php
                            $recibossat = $grupo->first();
                            $pagoIds = $grupo->pluck('pago.id')->filter()->unique()->implode(', ');
                        @endphp
                        <tr>
                            <td>{{ $recibossat ->nit }}</td>
                            <td>{{ $recibossat ->guid }}</td>
                            <td>{{ $recibossat ->serie }}</td>
                            <td>{{ $recibossat ->numero }}</td>
                            <td>{{ $pagoIds ?: 'N/A' }}</td>
                            <td>{{ \Carbon\Carbon::parse($recibossat ->created_at)->format('d/m/y') }}</td>
                            <td>
                                @if($recibossat ->link)
                                    <a href="{{ $recibossat ->link }}" target="_blank" class="btn btn-xs btn-info">
                            ...
                            <td>{{ $recibossat ->motivo }}</td>
                            <td>
                                <a href="{{ route('reportes.recibos.show', $recibossat->id) }}" class="btn btn-xs btn-primary">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center">No se encontraron recibos SAT.</td>
                        </tr>
                    @endforelse
      ...
      <form method="POST" id="form-anulacion" action="{{ route('anulador.enviar') }}">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="anularFacturaModalLabel{{ $recibossat->guid }}">Anular Recibo: {{ $recibossat->numero }}</h5>
          ...
          <div class="modal-body">
            <input type="hidden" name="numero_serie" value="{{ $recibossat->serie }}">
            <input type="hidden" name="numero_documento" value="{{ $recibossat->guid }}">
            <input type="hidden" name="nit_receptor" value="{{ $recibossat->nit ?? 'CF' }}">
            <input type="hidden" name="fecha_emision" value="{{ $recibossat->created_at }}">

            <div class="form-group">
              <label for="motivo">Motivo de Anulación</label>
              <textarea name="motivo" class="form-control" required placeholder="Escribe el motivo de anulación...">{{$recibossat->motivo}}</textarea>
            </div>
          </div>
