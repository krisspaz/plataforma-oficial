...
<div class="box box-default">
    ...
    <div class="box-header with-border">
        <h3 class="box-title">📄 Reporte de Facturas SAT Emitidos</h3>
    </div>

    <div class="box-body">
        {{-- Buscador por NIT --}}
        <form method="POST" action="{{ route('reportes.facturas.buscar') }}" class="form-inline mb-3">
            @csrf
            ...
            <a href="{{ route('reportes.facturas.index') }}" class="btn btn-default ml-2"><i class="fa fa-refresh"></i> Limpiar</a>
        </form>

  @php
    // Normalizar facturas activas
    $activos = collect($facturas)->map(function($r){
        return (object)[
            'nit' => $r->nit ?? null,
            'serie' => $r->serie ?? null,
    ...
    // Normalizar facturas anuladas
    $anulados = collect($facturasanuladas)->map(function($r){
        return (object)[
            'nit' => $r->nit ?? null,
            'serie' => $r->serie ?? null,
            'numero' => $r->numero ?? null,
                ...
                <tbody>
                    @forelse ($agrupadas as $numero => $grupo)
                        ...
                        >
                            ...
                            <td>
                                <a href="{{ route('reportes.facturas.show', $item->serie) }}" class="btn btn-xs btn-primary">
                                    <i class="fa fa-eye"></i> Detalle
                                </a>
                                <a href="{{ route('reportes.facturas.descargarPDF', $item->serie) }}" class="btn btn-xs btn-success">
    ...
    <div class="modal fade" id="anularModal{{ $numero }}" tabindex="-1" role="dialog" aria-labelledby="anularModalLabel{{ $numero }}" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('reportes.facturas.anular', $numero) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label>¿Desea anular esta Factura?</label><br>
