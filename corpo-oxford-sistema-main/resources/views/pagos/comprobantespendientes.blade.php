 ...
 <div class="panel panel-default p-4" style="border-radius: 12px; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
    ...
    <div class="panel-body">
        ...
        <div class="box-body">
            ...
            <div class="mb-3">
                <label for="buscarAlumno" class="form-label">Buscar Estudiante</label>
                <input type="text" class="form-control" id="buscarAlumno" placeholder="Escriba el nombre del estudiante..." style="width: 50%;">
            </div>
             <br>
            ...
            @foreach ($pagosAgrupados->groupBy('convenio_id') as $convenioId => $pagos)
                ...
                <div class="panel panel-default alumno-bloque" data-alumno="{{ strtolower($alumnos) }}">
        ...
        if (nit.length > 0) {
            try {
                ...
                if (data.success) {
                    ...
                    clienteInput.value = '';
                    alert("NIT o CUI no encontrado");
                }
            } catch (error) {
                console.error("Error al consultar:", error);
                alert("Hubo un error al consultar el identificador.");
                clienteInput.value = '';
            }
        } else {
            alert("Por favor ingrese un NIT o CUI válido.");
        }
    });
});
</script>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    inputBuscar.addEventListener('keyup', function () {
        ...
        document.querySelectorAll('.alumno-bloque').forEach(div => {
            const nombreAlumno = div.getAttribute('data-alumno');
            div.style.display = nombreAlumno.includes(valor) ? '' : 'none';
        });
    });
});
</script>
