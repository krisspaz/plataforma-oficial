...
<div class="panel panel-default">
    <div class="panel-heading">
        <h3>Convenios Con Gestión de Cuotas</h3>
    </div>
        ...
        <table class="table table-striped" id="datatable">
            <thead>
                <tr>
                    ...
                    <th>Acciones</th> <!-- Aquí hay 5 columnas -->
                </tr>
            </thead>
            <tbody>
                @forelse ($conveniosConDetalles as $convenio)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('convenios.mostrar_cuotas', $convenio->id) }}" class="btn btn-info">Ver Cuotas</a>
            
                            <!-- Botón de Eliminar -->
                            <form action="{{ route('convenios_detalles.destroy', $convenio->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" onclick="return confirm('¿Estás seguro de eliminar este registro?');">Eliminar</button>
                            </form>
                        </td> <!-- Esta columna 'Acciones' cierra correctamente la fila -->
                    </tr>
                @empty
