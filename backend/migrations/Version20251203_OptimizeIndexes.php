...
final class Version20251203_OptimizeIndexes extends AbstractMigration
{
    ...
    {
        return 'Optimiza índices para mejorar performance de queries frecuentes';
    }

    public function up(Schema $schema): void
    {
        // Índices compuestos para students
        $this->addSql('CREATE INDEX idx_students_search ON students USING gin(to_tsvector(\'spanish\', first_name || \' \' || last_name))');
        $this->addSql('CREATE INDEX idx_students_active ON students(status) WHERE status = \'active\'');
        $this->addSql('CREATE INDEX idx_students_grade_section ON students(grade_id, section_id)');

        // Índices para enrollments
        $this->addSql('CREATE INDEX idx_enrollments_academic_year ON enrollments(academic_year)');
        $this->addSql('CREATE INDEX idx_enrollments_student_year ON enrollments(student_id, academic_year)');
        $this->addSql('CREATE INDEX idx_enrollments_active ON enrollments(status) WHERE status = \'active\'');
        ...
        $this->addSql('CREATE INDEX idx_payments_student_status ON payments(student_id, status)');
        $this->addSql('CREATE INDEX idx_payments_created_at ON payments(created_at DESC)');

        // Índices para grades
        $this->addSql('CREATE INDEX idx_grades_student_subject ON grades(student_id, subject_id)');
        $this->addSql('CREATE INDEX idx_grades_period ON grades(grading_period_id)');
        $this->addSql('CREATE INDEX idx_grades_student_period ON grades(student_id, grading_period_id)');

        // Índices para attendance
        $this->addSql('CREATE INDEX idx_attendance_date ON attendance(attendance_date DESC)');
        $this->addSql('CREATE INDEX idx_attendance_student_date ON attendance(student_id, attendance_date)');
        $this->addSql('CREATE INDEX idx_attendance_status ON attendance(status)');

        // Índices para audit_logs
        $this->addSql('CREATE INDEX idx_audit_severity ON audit_logs(severity) WHERE severity IN (\'warning\', \'critical\')');
        $this->addSql('CREATE INDEX idx_audit_user_action ON audit_logs(user_id, action)');
        ...
        $this->addSql('CREATE INDEX idx_users_roles ON users USING gin(roles)');
    }
